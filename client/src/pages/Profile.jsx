...
async function fetchProfileData(userId) {
  const [profileRes, collectionRes, ratingsRes, wishlistRes, quizRes] = await Promise.all([
    ...
    supabase.from('user_ratings').select(`
      score, sillage, gender, season, review, created_at,
      perfumes ( perfume_id, name, image_url, brands ( brand_name ) )
...
export default function Profile() {
  ...
  const { profile, collection, ratings, wishlistCount, quiz } = data || {}
  ...
  const favSeason = (() => {
    const freq = {}
    ratings.forEach(r => { if (r.season) freq[r.season] = (freq[r.season] || 0) + 1 })
    const top = Object.entries(freq).sort((a, b) => b[1] - a[1])[0]
    return top ? SEASON_MAP[top[0]] : null
  })()
      ...
      <div style={styles.page}>
        ...
        <div style={styles.profileCard}>
          ...
          <div style={styles.profileInfo}>
            {editing ? (
              ...
              <>
                ...
                <button onClick={() => { setEditing(true); setForm({ name: profile?.name || '', age: profile?.age || '' }) }} style={styles.editBtn}>
                  Edit profile
                </button>
              </>
            )}
          ...
          <div style={styles.quickStats}>
            ...
            <QuickStat label="Ratings given" value={ratings.length}    />
                ...
                {ratings.map((r, i) => (
                  ...
                  >
                    ...
                    <div style={styles.ratingMeta}>
                      {r.score && <span style={styles.scoreChip}>{r.score}/10</span>}
                      {r.season && <span style={styles.metaChip}>{SEASON_MAP[r.season]}</span>}
...
function SectionHeader({ title, action }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
      ...
      {action && <Link to={action.to} style={{ fontSize: 12, color: '#C4845A', textDecoration: 'none', fontFamily: "'DM Sans', sans-serif", fontWeight: 500 }}>{action.label}</Link>}
    </div>
  )
}
...
const styles = {
  ...
  profileCard: { background: '#FDFAF6', border: '0.5px solid #E8DDD0', borderRadius: 16, padding: '24px', display: 'flex', alignItems: 'flex-start', gap: '1.5rem', marginBottom: '2rem', flexWrap: 'wrap' },
  avatarLarge: { width: 72, height: 72, borderRadius: '50%', background: '#C4845A', color: '#FDF8F2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 500, fontFamily: "'DM Sans', sans-serif", flexShrink: 0 },
  profileInfo: { flex: 1, minWidth: 0 },
  profileName: { fontFamily: "'Cormorant Garamond', serif", fontSize: 32, fontWeight: 400, color: '#2C2018', margin: '0 0 4px' },
  profileMeta: { fontSize: 13, color: '#9A8878', margin: '0 0 10px', fontFamily: "'DM Sans', sans-serif" },
  editBtn: { padding: '5px 14px', border: '0.5px solid #E0D4C4', borderRadius: 20, background: 'none', color: '#5C4A38', fontSize: 12, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" },
  ...
  saveBtn: { padding: '6px 14px', border: 'none', borderRadius: 20, background: '#C4845A', color: '#fff', fontSize: 12, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" },
  ...
  quickStats: { display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'flex-start' },
  qStat: { background: '#F0E8DC', borderRadius: 10, padding: '10px 16px', minWidth: 80 },
  qStatVal: { fontFamily: "'Cormorant Garamond', serif", fontSize: 26, color: '#C4845A', margin: '0 0 2px', lineHeight: 1 },
  qStatLabel: { fontSize: 11, color: '#9A8878', margin: 0, textTransform: 'uppercase', letterSpacing: '.05em', fontFamily: "'DM Sans', sans-serif" },
  ...
  ratingRow: { display: 'flex', alignItems: 'center', gap: 12, padding: '10px 8px', borderRadius: 8, textDecoration: 'none', transition: 'background .15s' },
  ratingThumb: { width: 36, height: 36, borderRadius: 6, background: '#EDE4D8', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
  ...
  ratingBrand: { fontSize: 11, color: '#9A8878', margin: 0, fontFamily: "'DM Sans', sans-serif" },
  ratingMeta: { display: 'flex', gap: 4, flexWrap: 'wrap', justifyContent: 'flex-end' },
  scoreChip: { fontSize: 11, padding: '2px 7px', background: '#EDE4D8', color: '#C4845A', borderRadius: 20, fontWeight: 500, fontFamily: "'DM Sans', sans-serif" },
  ...
  recLink: { fontSize: 12, color: '#C4845A', textDecoration: 'none', fontWeight: 500, fontFamily: "'DM Sans', sans-serif" },
